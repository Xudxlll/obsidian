...
> - 先通过 `input()` 获取用户输入，再根据题目需要做类型转换或合法性判断。
> - 使用条件判断把不同情况分开处理，让程序根据数据走不同分支。
> - 通过赋值语句保存中间结果，主要变量包括：`customer_type`、`amount`。
...
> - 先通过 `input()` 获取用户输入，再根据题目需要做类型转换或合法性判断。
> - 使用 `while` 形成持续循环，通常用于菜单、重复输入或未知次数的处理。
> - 通过赋值语句保存中间结果，主要变量包括：`total`、`count`、`grade`。
...
> [!abstract] 代码逻辑怎么走
> - 使用 `while` 形成持续循环，通常用于菜单、重复输入或未知次数的处理。
> - 通过赋值语句保存中间结果，主要变量包括：`height`、`n`、`total`。
...
> - 使用 `while` 形成持续循环，通常用于菜单、重复输入或未知次数的处理。
> - 使用条件判断把不同情况分开处理，让程序根据数据走不同分支。
> - 通过赋值语句保存中间结果，主要变量包括：`true_number`、`count`、`guess_number`。
...
> - 先通过 `input()` 获取用户输入，再根据题目需要做类型转换或合法性判断。
> - 使用 `for` 遍历序列或容器，对每个元素执行同样的处理。
> - 通过赋值语句保存中间结果，主要变量包括：`num1`、`total`。
...
> - 先通过 `input()` 获取用户输入，再根据题目需要做类型转换或合法性判断。
> - 使用条件判断把不同情况分开处理，让程序根据数据走不同分支。
> - 通过赋值语句保存中间结果，主要变量包括：`distance`、`money`。
