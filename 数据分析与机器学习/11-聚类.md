...
噪声密度（Density-Based Spatial Clustering of Applications with Noise， 简写DBSCAN，[[../机器、深度学习额外笔记/DBSCAN 聚类算法介绍与实际应用示例|DBSCAN 聚类算法介绍与实际应用示例]]），==随机选择一个样本==做圆心，以事先给定的半径做圆，凡被该圆圈中的样本都被划为与圆心样本同处一个聚类，再以这些被圈中的样本做圆心，以事先给定的半径继续做圆，不断加入新的样本，扩大聚类的规模，直到再无新的样本加入为止，即完成一个聚类的划分。以同样的方法，在其余样本中继续划分新的聚类，直到样本空间被耗尽为止，即完成整个聚类划分过程。示意图如下：
...
3. complete 链接（**全链路**）：也称为最大链接，通过比较两个簇中所有点之间的最大距离来确定簇间距离的。合并两个簇中最不相似的样本（即距离最远的样本）所属的簇，==最大距离最小的两个簇合并==。
