...
## 1. Grid search 与 random search 的问题
...
这一讲对应 PDF：p.1-p.66。它把第一讲的 MLP 具体放进回归任务（Regression）里，重点不是“再学一种模型”，而是理解深度学习训练的基本形式：**经验风险最小化（Empirical Risk Minimization, ERM）**。

推荐把本讲分成两条线阅读：

1. p.7-p.33：从线性回归的闭式解转向神经网络的数值优化。
2. p.34-p.60：从普通平方损失转向稳健损失（Robust Loss）和分位数回归（Quantile Regression）。
...
但深度神经网络的 $f_\theta(x)$ 是多层非线性复合函数，损失函数通常不是简单凸函数（Convex Function）。因此训练目标更常写成：
...
- $f_\theta$ 是模型，即神经网络本身；
- $\ell$ 是单个样本的损失（Loss），决定“错得多严重”；
- 平均损失是训练集上的经验风险（Empirical Risk）。

p.15-p.33 讲梯度时，建议你不要只背“沿负梯度方向下降”。更直观的理解是：梯度 $\nabla L(\theta)$ 指向损失上升最快的方向，所以 $-\nabla L(\theta)$ 是局部下降最快的方向。一次梯度下降更新可以写成：

$$
\theta_{t+1}=\theta_t-\eta\nabla L(\theta_t)
$$

其中 $\eta$ 是学习率（Learning Rate）。$\eta$ 太小会学得慢，太大可能越过低谷甚至发散。

p.34-p.60 的 robust regression 要抓住“损失函数决定模型更相信什么”。平方损失（Least Squares, LS）对大残差惩罚非常重：

$$
\ell(r)=r^2
$$

所以异常值（Outlier）会强烈拉动模型。LAD 损失（Least Absolute Deviation）使用：

$$
\ell(r)=|r|
$$

它对应条件中位数（Conditional Median），比均值更不容易被极端值拖走。分位数损失（Quantile Loss）进一步把目标从“中位数”推广到任意分位数：

$$
\rho_\tau(r)=r(\tau-\mathbf{1}\{r<0\})
$$

这里 $\tau$ 越大，模型越关注上分位；$\tau$ 越小，模型越关注下分位。它不是普通回归的附属知识，而是在金融、风险管理、区间预测中非常常用的思想。

本讲复习时可以用一个判断规则：**先问自己预测目标是什么统计量，再选择对应损失函数。**预测均值用平方损失，预测中位数用绝对值损失，预测某个分位点用 quantile loss。
