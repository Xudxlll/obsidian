...
## 3. Forward pass 与 backward pass
...
这一讲对应 PDF：p.1-p.50。它解决第二讲留下的核心问题：神经网络参数很多，损失函数又是复合函数，我们到底怎样高效得到梯度，并怎样用梯度更新参数？

读本讲建议按三步：

1. p.6-p.19：先理解反向传播（Backpropagation）本质上是链式法则（Chain Rule）的系统化应用。
2. p.22-p.34：再理解全批量梯度下降（Gradient Descent）和随机梯度下降（Stochastic Gradient Descent, SGD）的差别。
3. p.35-p.50：最后看 Momentum、AdaGrad、RMSProp、ADAM 如何修正普通 SGD 的不足。

反向传播不是一个“神秘算法”，而是把复合函数拆成计算图（Computational Graph）。如果有：
...
反向传播高效的地方在于：中间层的局部导数可以被复用，不需要对每个参数从头计算一次完整导数。

对应 p.13-p.19 的 one-layer perceptron example，要重点看 loss 对输出、输出对激活、激活对线性组合、线性组合对权重的链条。一般可以按下面顺序读：

- 先写前向传播（Forward Pass）：输入如何一步步变成预测值；
- 再写损失（Loss）：预测值和真实值如何比较；
- 最后写反向传播（Backward Pass）：损失如何一层层传回参数。

p.28-p.34 的 SGD 要和 GD 对照理解。全梯度使用所有样本：

$$
\nabla L(\theta)=\frac{1}{n}\sum_{i=1}^n \nabla \ell_i(\theta)
$$

SGD 或 minibatch SGD 只用一小批样本估计这个方向：

$$
g_t=\frac{1}{|B_t|}\sum_{i\in B_t}\nabla \ell_i(\theta_t)
$$

它的好处是每一步便宜，坏处是方向有噪声（Noise）。因此实际训练中常见 minibatch、epoch、learning rate schedule，这些不是工程细节，而是为了在计算成本和梯度质量之间折中。

本讲优化算法可以这样记：

| 方法 | 想解决的问题 | 直观理解 |
|---|---|---|
| Momentum | SGD 抖动、峡谷中前进慢 | 给更新方向加“惯性” |
| Nesterov | Momentum 可能冲过头 | 先向前看一眼再修正 |
| AdaGrad | 不同参数尺度不同 | 常更新的参数步子变小 |
| RMSProp | AdaGrad 学习率衰减太快 | 用滑动平均代替无限累积 |
| ADAM | 想同时要动量和自适应步长 | 一阶动量 + 二阶动量 |

复习时不要只背算法名，要能回答：**它修改的是梯度方向、步长，还是梯度的统计估计？**
