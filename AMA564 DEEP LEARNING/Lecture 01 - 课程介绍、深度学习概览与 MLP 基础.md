...
## 3. LeNet、第二次 AI winter 与 2006 年后的复兴
...
这一讲对应 PDF：p.3-p.89。读的时候不要把它只看成课程介绍，它其实在建立后面所有模型的共同语言：**数据进入模型，模型产生预测，损失函数衡量误差，优化算法更新参数**。p.3-p.5 给出课程主题，p.64-p.89 则把这条主线落到多层感知机（Multi-Layer Perceptron, MLP）上。

**推荐阅读顺序**：

1. 先看 p.3-p.5，知道本课反复出现的四类对象：网络结构、学习算法、任务类型、实践工具。
2. 再看 p.64-p.77，把 MLP 当成一个复合函数（composite function）来理解，而不是只记“很多层神经元”。
3. 最后看 p.78-p.89，理解为什么“深”不只是把层数堆高，而是和表达效率（representation efficiency）有关。

当你复习本讲时，可以用一句话串起来：**深度学习就是用带参数的多层函数族 $f_\theta$ 去拟合复杂关系，再用数据和优化方法找到合适的参数 $\theta$。**

本讲最容易混的是“深度学习（Deep Learning）”“神经网络（Neural Network）”和“MLP”三者的范围。

- **深度学习（Deep Learning）**：是一类机器学习方法，核心是通过多层表示学习（Representation Learning）从数据中自动学习特征。
- **神经网络（Neural Network）**：是实现深度学习的主要函数族，但不同网络结构服务于不同数据形态。
- **MLP（Multi-Layer Perceptron）**：是最基础的前馈神经网络（Feedforward Neural Network），它适合用来理解“层、参数、激活函数、复合函数”这些核心概念。

对应 p.67-p.77 的 MLP 公式，最重要的是看清每一层都在做两步：
...
第一步是仿射变换（Affine Transformation），负责把上一层表示重新组合；第二步是激活函数（Activation Function），负责引入非线性。如果没有 $\sigma$，多层线性变换仍然可以合并成一层线性变换：

$$
W_2(W_1x+b_1)+b_2=(W_2W_1)x+(W_2b_1+b_2)
$$

所以 activation 不是装饰，而是 MLP 能拟合复杂函数的关键。

复习 p.78-p.89 时，要把“万能逼近（Universal Approximation）”和“实际可训练”区分开。万能逼近告诉我们存在某个网络能逼近目标函数，但并不保证：
...
因此后续课程会继续引入损失函数、反向传播、优化算法、CNN、Transformer 等内容。它们都在回答同一个问题：**怎样让神经网络既表达得出、又训练得动、还能泛化到新数据。**
