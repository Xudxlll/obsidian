...
内置作用域（`Built-in Scope`）指`Python` 解释器预定义的名称空间，包含内置函数和异常等，这些名称在所有 `Python` 程序中均可使用，无需导入。
...
递归函数(`Recursive`)是指在函数定义中直接或兼接调用自身的函数。
...
装饰器（`Decorator`）是 `Python` 的一种语法特性和设计模式，它允许通过 `@` 语法将一个可调用对象（函数或类）包装在另一个可调用对象中，以便在不修改原始代码的情况下扩展或修改其行为。
