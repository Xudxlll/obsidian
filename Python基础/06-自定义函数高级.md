...
这两个都是典型的高阶函数——第一个参数接收的都是**函数对象**（函数名不带括号），而不是调用结果。

> 更多用法见 [[06-自定义函数高级#5.2.1 `map()`函数|5.2.1 map()]] 和 [[06-自定义函数高级#5.2.2 `filter()`函数|5.2.2 filter()]]。
...
# calculate_total = log_execution(calculate_total)
