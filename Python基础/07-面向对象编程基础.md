...
● 类定义：使用 `class` 关键字定义类，`ClassName` 是类名，并且使用大驼峰命名法（`PascalCase`）。
...
<img src="assets/20260414.png" style="border:1px solid #ccc;padding:10px;border-radius:10px;width:50%"/>
...
`@property`装饰器用于将一个方法转换为只读属性。可以让方法像属性一样被访问，而不需要显式调用（即不需要加括号）。
