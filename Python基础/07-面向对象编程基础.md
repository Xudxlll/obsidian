...
class Circle:
    def __init__(self, radius):
        self.radius = radius  # 通过 setter 赋值，自动触发验证
...
# c2 = Circle(-4)  # ValueError: 半径必须是正数
...
**注意**：在 `__init__` 中应该使用 `self.radius = radius`（走 setter），而不是 `self._radius = radius`（绕开验证）。由于 setter 内部实际存储的仍是 `self._radius`，因此底层数据依然是"私有"的，封装性没有被破坏——`radius` 是公开接口，`_radius` 是私有存储，两者各司其职。
...
### 3.3.3 类属性与实例属性同名时的访问规则

当类属性和实例属性使用同一个名字时，容易产生混淆。可以先记住一个核心规则：

> **对象访问属性时，会优先在对象自己的实例属性中查找；如果对象自己没有这个属性，才会去类属性中查找。**
...
class Person:
    ...
    def __init__(self, age):
        self.name = '吴华'  # 实例属性，与类属性同名
        self.age = age     # 实例属性，对象属性

"""
如何访问类属性？

1. 通过对象访问：
   因为类属性是所有对象共享的，所以对象可以访问类属性。

2. 通过类名访问：
   如果实例中也有同名属性，想明确访问类属性，就应该使用 类名.属性名。
"""

p1 = Person(age=26)
p2 = Person(age=27)

print(p1.name)      # 吴华，此时访问的是实例属性，因为 p1 自己有 name
print(Person.name)  # 张三，通过类名访问类属性
```

在这个例子中，`Person` 类里有一个类属性：

```python
name = '张三'
```

同时，在 `__init__` 方法中又给每个对象创建了一个实例属性：
...
1. 先看 `p1` 这个对象自己有没有 `name`；
2. 发现 `p1` 自己有 `self.name = '吴华'`；
3. 所以直接返回 `'吴华'`，不会继续去类里面找 `Person.name`。
...
类属性确实可以被所有对象访问，但如果通过对象给同名属性重新赋值，通常不是修改类属性，而是给这个对象自己新增或修改一个实例属性。
...
这行代码修改的是 `p1` 自己的实例属性 `name`，不是修改 `Person.name`。

所以此时：

```python
print(p1.name)      # 李四，p1 自己的实例属性
print(p2.name)      # 吴华，p2 自己的实例属性
print(Person.name)  # 张三，类属性没有变
```
...
但是要注意：如果某个对象已经有了同名实例属性，那么它访问到的仍然是自己的实例属性，而不是类属性。
...
因为 `p1` 和 `p2` 都已经有自己的 `name` 实例属性，所以它们会优先使用自己的 `name`。
...
```text
先找对象自己的实例属性 → 找不到再去类里面找类属性
```

所以：

- `p1.name`：优先访问 `p1` 自己的实例属性；
- `Person.name`：直接访问类属性；
- `p1.name = '李四'`：修改或创建的是 `p1` 的实例属性；
- `Person.name = '王五'`：修改的是类属性。
...
class Scholar:  
    ...
    def eat(self,something):  
        print(f'TA正在吃{something}')  
  
class Teacher(Scholar):  
    def __init__(self,name,sex,age,course_name,):  
        self.course_name = course_name  
        super().__init__(name,age,sex)  
  
    def teach(self):  
        print(f'TA在教{self.course_name}课程')  
  
#创建一个老师  
t1 = Teacher('物理','赵六',36,'男')

```

![](assets/07-面向对象编程基础/file-20260610170505533.png)
