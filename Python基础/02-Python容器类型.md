...
# 因为步长值为负值，所以此时Python解释器会把起始值默认为最后一个成员，结束值默认为第一个成员---倒序或者称为元组的反转
...
| a, *b, c         | b 收集中间元素  |
| *a, b            | a 收集前面元素  |
| first, *_ , last | 取首尾，忽略中间  |
| *a, = iterable   | 收集全部元素到列表 |
...
字典（`dict`）是 `Python` 的内置**可变的、映射类型**（`mutable mapping`），用于存储键值对（`key-value pairs`），并通过键来访问对应的值。ps：==字典是无序的==。
...
| 是否属于对象  | 否                                 | 是                            |
| 调用方式    | 函数名(参数)                           | 对象.方法名()                     |
| 示例      | len()、str()、int()、==enumerate()== | append()、==items()==、upper() |
| 使用范围    | 通常适用于多种对象                         | 通常只适用于特定对象                   |
| 本质      | 独立的功能                             | 对象自带的功能                      |

记住一句话：**函数是独立工具，方法是对象自带的工具。**
...
# 3) dict.update(**kwargs)：用关键字参数更新（键必须是合法标识符，键不能是变量）
d.update(e=5, f=6)
