...
<img src="assets/03.png"  style="width:60%;padding:5px;border:1px dotted #ccc;border-radius:5px;" />

<img src="assets/04.jpg"  style="width:60%;padding:5px;border:1px dotted #ccc;border-radius:5px;"/>
...
<img src="assets/05.png"  style="width:60%;padding:5px;border:1px dotted #ccc;border-radius:5px;"/>

<img src="assets/06.png" style="width:60%;padding:5px;border:1px dotted #ccc;border-radius:5px;"/>
...
<img src="assets/07.png"  style="width:60%;padding:5px;border:1px dotted #ccc;border-radius:5px;" />
...
环境变量是操作系统中用于存储信息的一种机制，它可以向应用程序传递配置参数或系统信息。

环境变量通常以键值对的形式存在，键是变量名，值是变量的内容。这些变量可以影响程序的行为或提供运行时所需的信息。

**环境变量的用途**

● 配置程序运行环境：例如，指定程序的日志路径、数据库连接信息等。

● 操作系统信息：存储系统级信息，例如用户目录、主机名等。

● 跨平台支持：使用环境变量可以避免硬编码路径或配置，从而提高程序的可移植性。

● 安全性：将敏感信息（如 `API` 密钥、密码）存储在环境变量中，而不是硬编码在代码中。

**设置环境变量**

右键“此电脑”→“属性”→“高级系统设置”→“环境变量”，然后在“系统变量”或“用户变量”中添加或修改变量。

<img src="assets/env.png" style="border:1px solid #ccc;padding:10px;border-radius:5px;"/>

**关于 `PATH` 环境变量的说明**

`PATH` 是操作系统中的一个重要环境变量，用于指定可执行文件的搜索路径。

当用户在终端中输入命令或运行程序时，系统会根据 `PATH` 环境变量中的路径查找对应的可执行文件。

**`PATH` 环境变量的工作原理**

1.当用户输入命令时，系统会按照 `PATH` 中的路径顺序依次查找对应的可执行文件。

2.如果在某个路径中找到匹配的文件，系统会执行该文件。

3.如果所有路径中都找不到该文件，系统会提示命令未找到（`Linux/Unix`）或显示错误（`Windows`）。

### 1.5.4 关于`pip`的补充 

`pip` 是 `Python` 的包管理工具，名字来源于 `Pip Installs Packages`，它用于安装、管理、更新和卸载 `Python` 的软件包。

`pip` 是 `Python` 开发者必备的工具之一，可以帮助轻松获取和管理 `Python` 的第三方库。
...
从 `Python` 包管理平台 [`PyPI` (`Python Package Index`)](https://pypi.org/) 下载并安装所需的库。

<img src="assets/20260428.png" style="border:1px solid #ccc;padding:5px;border-radius:5px;"/>
...
第一步：启动`Windows`命令行(`Win+R`，然后输入 `cmd`)

<img src="assets/cmd.png" style="padding:5px;border:1px dotted #ccc;border-radius:5px;" />

第二步：在`Windows`命令行中输入`python`：

<img src="assets/python.png"  style="padding:5px;border:1px dotted #ccc;border-radius:5px;max-width:70%"/>
...
`PyCharm` 是一款专业的 `Python` 集成开发工具（`IDE`），支持 `MacOS`、`Windows` 和 `Linux` 系统。

需要注意的是，`PyCharm` 本身并不自带 `Python` 解释器，用户需要在系统中预先安装 `Python` 并在 `IDE` 中配置相应的解释器。
...
变量通常由==变量名称==、==变量值==和==数据类型==组成。

==变量名称用于标识变量，变量值是存储在变量中的数据，而数据类型决定了变量存储的数据类型及其占用的内存大小。==

变量名称是**对象的引用/绑定**，对象才有类型，变量名称本身没有**固定类型**。

<img src="assets/image_b6e3105f.png" style="border:1px solid #ccc;padding:10px;border-radius:5=10px;"/>
...
<img src="assets/datatype.png" style="border:1px solid #ccc;padding:10px;border-radius:10px;"/>
...
> 在`Python`中，浮点数存储可能会存在合理误差，因此==尽量避免对浮点数进行相等性比较操作。==
>
> 在`Python`中，对==浮点数进行大小比较通常是安全的。==
...
<img src="assets/typewriter.jpg" style="border:1px solid #ccc;padding:10px;border-radius:10px;"/>
...
<img src="assets/memory_allocate.png" style="border:1px solid #ccc;padding:10px;border-radius:5px;"/>

在 `Python` 中，变量声明的过程可以分为以下几个步骤，每一步都涉及到 `Python` 的内存管理和对象模型：

**1. 创建对象**

当一个变量被赋值时，`Python` 首先会创建一个对象，存储在内存中。对象的创建包括：

**分配内存**：为对象分配内存空间。

**初始化对象**：根据赋值的值，初始化对象的类型和内容。

比如，`x = 10` 会创建一个 `int` 类型的对象，值为 `10`。

如果赋值的值已经存在，并且是不可变类型（如 `int`、`str`、`tuple`），`Python` 可能会复用已有的对象，而不会创建新对象（例如，小整数缓存机制）。

**2. 创建变量并绑定对象**

`Python` 中的变量本质上是一个**名称**，它存储了对对象的引用。

当你声明 `x = 10` 时，`Python` 会将变量 `x` 绑定到刚刚创建的整数对象 `10`。

变量只是一个指针，指向内存中的对象。

**3. 更新对象的引用计数**

`Python` 的每个对象都有一个**引用计数器**，用于记录有多少变量或其他对象引用了它。

当一个变量被绑定到对象时，`Python` 会将该对象的引用计数加 1。
...
在这种情况下，`10`的引用计数会增加到 2。

**4. 垃圾回收机制**

如果一个对象不再被任何变量或其他对象引用（即引用计数降为 0），`Python` 的垃圾回收器会自动将其从内存中删除，以释放资源。
...
x = 10
x = 20  # 原来的 10 的引用计数变为 0，可能会被回收

```

**5. 特殊情况**

**不可变对象**：如 `int`、`str`、`tuple`，这些对象一旦创建，内容不能更改。如果重新赋值，`Python` 则会创建新的对象。

```python

x = 10
x = 20  # 创建新的对象 20，并将 x 指向它

```

**可变对象**：如 `list`、`dict`、`set`，可以在原地修改对象，而不会创建新的对象。

```python

lst = [1, 2, 3]
lst.append(4)  # 修改了原对象，未创建新对象

```

# 六、数据类型转换

在`Python`中，数据类型转换可以分为：隐式转换（自动转换）和显式转换（强制转换）。
...
| 函数                     | `描述`              |
| :--------------------- | :---------------- |
| `int(string=0,/)`      | 将`string`转换为整数    |
| `float(string,/)`      | 将`string`转换为浮点数   |
| `complex(string)`      | 将`string`转换为复数    |
| `str(object)`          | 将 `object` 转换为字符串 |
| `bool(object=False,/)` | 将 `object` 转换为布尔型 |
| `bin(integer,/)`       | 将`integer`转换为二进制  |
