...
#以下演示为读取文件操作，在本案例中想提醒大家的是：文件对象在读取时也是返回的迭代器，每次只读一行
...
g = test()   # 必须让 `g` 接收 `test()`，是因为：`test()` 返回的是生成器对象，生成器对象               负责保存执行状态；后续多次 `next(g)` 才能从上次 `yield` 暂停的位置继续执行。
...
```
![](assets/05-生成器与迭代器和自定义函数基础/file-20260605155914095.png)
...
def my_sum2(iterable,/,*,default,key=None):

print(iterable,default,key)

# my_sum2((1,2,3)) #因为必选参数没有赋值

# my_sum2((1,2,3),"AAA") #因为必须按关键字参数进行赋值

my_sum2((1,2,3),default="BBB")
...
可变关键字形参是指由任意数量的关键字参数，它需要通过在形参名称加前缀 `**` 来定义
